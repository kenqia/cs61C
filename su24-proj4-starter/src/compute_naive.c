...
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  ...
  int down, right;
  down = right = 0;
  while(down < rslt_matrix->rows && right < rslt_matrix->cols){
    int sum = 0;
    for(int i = 0 ; i < b_rows ; i++){
      for(int j = 0 ; j < b_cols ; j++){
        sum += a_matrix->data[(i + down)* a_cols + j + right] * tmp_data[i * b_cols + j];
      }
    }
    rslt_matrix->data[down * rslt_matrix->cols + right] = sum;
    right++;
    if(right >= rslt_matrix->cols){
      right = 0;
      down++;
    }
  }
